...
trait Neo4jWrapper extends GraphDatabaseServiceProvider with Neo4jWrapperImplicits {
  ...
  def serialize[T <: PropertyContainer](cc: AnyRef, pc: PropertyContainer): T = {
    CaseClassDeserializer.serialize(cc).foreach {
      case (name, null) =>
      case (name: String, value: List[AnyRef]) => //handleList(pc, name, value)
      case (name: String, value: AnyRef) => handleRef(pc, name, value)
      case (name, value) => pc.setProperty(name, value)
    }
    pc(Neo4jWrapper.ClassPropertyName) = cc.getClass.getName
    pc.asInstanceOf[T]
  }

  private def handleList(pc:PropertyContainer, name:String, ccs:List[AnyRef]) {
    val source = pc.asInstanceOf[Node]
    ccs.foreach { cc =>
      val n = createNode(ds)
      serialize(cc, n)
      source.createRelationshipTo(n, name)
    }
  }

  private def handleRef(pc:PropertyContainer, name:String, value:Any) {
    value match {
      case _:Int => pc.setProperty(name, value)
      case _:Long => pc.setProperty(name, value)
      case _:String => pc.setProperty(name, value)
      case _ =>
        // val source = pc.asInstanceOf[Node]
        // val n = createNode(ds)
        // serialize(value.asInstanceOf[AnyRef], n)
        // source.createRelationshipTo(n, name)
    }
  }
}
