...
class RestTests extends MutableScalatraSpec {
  ...
  implicit val jsonFormats = Serialization.formats(ShortTypeHints(List(classOf[Post], classOf[Page], classOf[Category], classOf[Tag]))) + FieldSerializer[Post]()  + FieldSerializer[Gallery]() + FieldSerializer[Image]()
...
//     }
     ...
     "should find tags for post" in {
       get("/api/posts/%s/tags".format(1)) {
         status must beEqualTo(200)
         val tags = parse(body).extract[List[Tag]]
